...
function Berichten() {
  ...
  const [geplandeBerichten, setGeplandeBerichten] = useState<{ tekst: string; datum: string; chatId: string }[]>([]);
    ...
    <AppShell title="Berichten" subtitle="Veilige, versleutelde communicatie">
      <div className="grid gap-0 overflow-hidden rounded-2xl border border-border bg-card md:grid-cols-[320px_1fr]" style={{ height: "calc(100vh - 10rem)" }}>
        ...
        <section className={`min-w-0 flex-col md:flex ${mobileDetail ? "flex" : "hidden"}`}>
          ...
          >
            {role === "docent" && geplandeBerichten.filter((b) => b.chatId === chat.id).length > 0 && (
              <div className="border-t border-border bg-muted/30 px-3 py-2">
                <button type="button" onClick={() => setGeplandOpen((v) => !v)} className="inline-flex items-center gap-1 text-xs font-semibold text-primary">
                  <CalendarClock className="h-3.5 w-3.5" /> Ingeplande berichten: {geplandeBerichten.filter((b) => b.chatId === chat.id).length}
                </button>
                {geplandOpen && (
                  <div className="mt-2 space-y-1">
                    {geplandeBerichten
                      ...
                      .map((b) => (
                        <div key={b.idx} className="flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-1.5 text-xs">
                          <span className="flex-1 truncate">{b.tekst}</span>
                          <span className="shrink-0 text-muted-foreground">{new Date(b.datum).toLocaleString("nl-NL", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</span>
                          <button type="button" onClick={() => setGeplandeBerichten((prev) => prev.filter((_, j) => j !== b.idx))} className="shrink-0 text-destructive hover:text-destructive/80">×</button>
                        </div>
                      ))}
                  </div>
                )}
            ...
            {role === "docent" && planOpen && (
              <div className="flex flex-wrap items-center gap-2 rounded-lg bg-muted/60 px-3 py-2">
                <input type="datetime-local" value={planDatum} onChange={(e) => setPlanDatum(e.target.value)} className="rounded-lg border border-border bg-background px-2 py-1 text-xs outline-none focus:border-primary" />
                <button type="button" disabled={!planDatum || !msg.trim()} onClick={() => {
                  setGeplandeBerichten((prev) => [...prev, { tekst: msg.trim(), datum: planDatum, chatId: chat.id }]);
                  setMsg("");
                  setPlanOpen(false);
                  setPlanDatum("");
                  toast.success("Bericht ingepland");
                }} className="rounded-lg bg-primary px-2 py-1 text-xs font-semibold text-primary-foreground disabled:opacity-50">Inplannen</button>
                <button type="button" onClick={() => setPlanOpen(false)} className="text-xs text-muted-foreground hover:text-foreground">Annuleren</button>
              </div>
            )}
            <div className="flex items-center gap-2">
              ...
              {role === "docent" && (
                <button type="button" onClick={() => setPlanOpen((v) => !v)} className={`rounded-lg p-2 hover:bg-muted ${planOpen ? "bg-muted text-primary" : ""}`} aria-label="Inplannen">
      ...
      {nieuwChatOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setNieuwChatOpen(false)}>
          <div className="w-full max-w-md overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-border p-4">
              ...
              <button onClick={() => setNieuwChatOpen(false)} className="rounded-lg p-1.5 hover:bg-muted"><X className="h-4 w-4" /></button>
            </div>
            <div className="space-y-3 p-4">
              <div className="flex items-center gap-2 rounded-lg bg-muted/60 px-3 py-2">
                <Search className="h-4 w-4 text-muted-foreground" />
                <input value={ncSearch} onChange={(e) => setNcSearch(e.target.value)} placeholder="Zoek persoon..." className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
              </div>
              <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-border p-2.5">
                <input type="checkbox" checked={ncGroep} onChange={(e) => setNcGroep(e.target.checked)} className="accent-primary" />
                <div className="text-xs font-semibold">Groepschat aanmaken</div>
              </label>
              {ncGroep && (
                <input value={ncGroepNaam} onChange={(e) => setNcGroepNaam(e.target.value)} placeholder="Naam van de groep" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
              )}
              ...
              <div className="max-h-48 space-y-1 overflow-y-auto">
                {contacten.filter((c) => !ncSearch.trim() || c.naam.toLowerCase().includes(ncSearch.toLowerCase())).map((c) => (
                  <label key={c.id} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-2.5 transition-colors ${ncSelected.includes(c.id) ? "border-primary bg-primary/5" : "border-border"}`}>
                    <input type="checkbox" checked={ncSelected.includes(c.id)} onChange={(e) => setNcSelected((s) => e.target.checked ? [...s, c.id] : s.filter((x) => x !== c.id))} className="accent-primary" />
                    <div className="min-w-0 flex-1">
            ...
            <div className="flex justify-end gap-2 border-t border-border p-3">
              <button onClick={() => setNieuwChatOpen(false)} className="rounded-lg border border-border px-3 py-2 text-sm">Annuleren</button>
              <button
                disabled={ncSelected.length === 0}
                onClick={() => {
                  const personen = ncSelected.map((id) => contacten.find((c) => c.id === id)!).filter(Boolean);
                  const newId = `nc-${Date.now()}`;
                  const titel = ncGroep ? (ncGroepNaam.trim() || `Groep (${personen.map((p) => p.naam.split(" ")[0]).join(", ")})`) : personen[0].naam;
                  ...
                  setNcGroepNaam("");
                  setNcSelected([]);
                  toast.success(ncGroep ? `Groepschat "${titel}" aangemaakt` : `Gesprek met ${titel} gestart`);
                }}
