  ...
  return (
    <AppShell title="Opdrachten" subtitle="Beheer, beoordeel en controleer opdrachten">
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
        <StatBtn active={filter === "openstaand"} onClick={() => setFilter(filter === "openstaand" ? "alle" : "openstaand")} icon={Clock} label="Openstaand" value={counts.openstaand} tone="warning" />
      ...
      {filter === "te-laat" && (
        <Card title="Te laat ingeleverd" className="mb-6">
          {items.filter((i) => i.telaatLeerlingen?.length).length === 0 ? (
            <div className="p-4 text-center text-sm text-muted-foreground">Geen te-late inleveringen</div>
          ) : (
            <div className="overflow-hidden rounded-xl border border-border">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
                  <tr><th className="px-4 py-2 text-left">Leerling</th><th className="px-4 py-2 text-left">Opdracht</th><th className="px-4 py-2 text-right">Te laat</th></tr>
                </thead>
                <tbody>
                  {items.flatMap((o) => (o.telaatLeerlingen ?? []).map((l, i) => (
                    <tr key={o.id + i} className="border-t border-border">
                      <td className="px-4 py-3 font-medium">{l.naam}</td>
                      <td className="px-4 py-3 text-muted-foreground">{o.titel} · {o.klas}</td>
                      <td className="px-4 py-3 text-right"><span className="rounded-full bg-warning/15 px-2 py-0.5 text-xs font-semibold text-warning">{l.dagenTeLaat}d</span></td>
                    </tr>
                  )))}
                </tbody>
              </table>
      ...
      <Card title="Opdrachten" action={
        <button onClick={() => setNieuwOpen(true)} className="inline-flex items-center gap-1 rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground">
          <Plus className="h-3.5 w-3.5" /> Nieuwe opdracht
        </button>
      }>
        <div className="space-y-3">
          {zichtbaar.length === 0 && <div className="rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Geen opdrachten in "{filter}"</div>}
          {zichtbaar.map((o) => (
            <div key={o.id} className="rounded-xl border border-border bg-background p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-semibold">{o.titel}</div>
                  <div className="mt-0.5 text-xs text-muted-foreground">{o.vak} · {o.klas} · deadline {o.deadline} · weging ×{o.weging}</div>
                  <div className="mt-2 text-xs text-muted-foreground">Ingeleverd: <strong className="text-foreground">{o.ingeleverd}</strong> / {o.totaal}</div>
                </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border p-4">
          <div className="text-sm font-semibold">{init ? "Opdracht bewerken" : "Nieuwe opdracht"}</div>
          <button onClick={onClose} className="rounded-lg p-1.5 hover:bg-muted"><X className="h-4 w-4" /></button>
        </div>
        <div className="space-y-3 p-4">
          <input value={titel} onChange={(e) => setTitel(e.target.value)} placeholder="Titel" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
          <div className="grid grid-cols-2 gap-2">
            <input value={vak} onChange={(e) => setVak(e.target.value)} placeholder="Vak" className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
            <input value={klas} onChange={(e) => setKlas(e.target.value)} placeholder="Klas" className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
          </div>
          <input value={deadline} onChange={(e) => setDeadline(e.target.value)} placeholder="Deadline (bv. 5 dec 23:59)" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
          <label className="block">
            <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Weging</span>
            <input type="number" min="1" value={weging} onChange={(e) => setWeging(e.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
          </label>
...
function AICheckModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border p-4">
          <div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /><span className="text-sm font-semibold">AI-controle</span></div>
          <button onClick={onClose} className="rounded-lg p-1.5 hover:bg-muted"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-6">
          <div className="mb-2 flex items-baseline justify-between">
            <div className="text-xs text-muted-foreground">{done ? "Analyse voltooid" : "Bezig met analyseren…"}</div>
            <div className="text-2xl font-bold tabular-nums">{progress}%</div>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
            <div className="h-full rounded-full bg-gradient-to-r from-primary to-primary/70 transition-[width] duration-100" style={{ width: `${progress}%` }} />
          </div>
          {done && (
            <div className="mt-6 space-y-2 text-sm">
              <div className="rounded-lg bg-success/10 p-3 text-success"><ShieldAlert className="mr-2 inline h-4 w-4" /> 18 opdrachten origineel</div>
              <div className="rounded-lg bg-warning/10 p-3 text-warning"><AlertTriangle className="mr-2 inline h-4 w-4" /> 2 verdachte overeenkomsten (42% en 38%)</div>
              <div className="rounded-lg bg-muted p-3 text-muted-foreground text-xs">AI-detectie: geen indicatie van AI-gegenereerde tekst.</div>
            </div>
          )}
        </div>
        <div className="flex justify-end border-t border-border p-3">
          <button onClick={onClose} className="rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground">{done ? "Sluiten" : "Annuleren"}</button>
        </div>
      </div>
    </div>
  );
