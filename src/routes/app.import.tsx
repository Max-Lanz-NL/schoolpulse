  ...
  if (role !== "directie") {
    return (
      <AppShell
        ...
        subtitle="Importeer leerlingendata vanuit Magister of Somtoday"
      >
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border bg-card p-12 text-center">
          ...
          <div className="text-sm font-semibold text-muted-foreground">
            Alleen beschikbaar voor directie.
          </div>
        </div>
      </AppShell>
    );
...
function ImportWizard() {
  ...
  const laadVoorbeeldData = (b: Bron) => {
    ...
    if (b === "magister") {
      ...
      setMapping({
        leerlingnr: "Leerlingnummer",
        naam: "Naam",
        klas: "Klas",
        geboortedatum: "Geboortedatum",
        ouder_email: "Ouder e-mail",
      });
    } else if (b === "somtoday") {
      ...
      <div className="mb-6 flex items-center gap-2">
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex items-center gap-2">
            <div
              className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${stap >= s ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
            >
              {s}
            </div>
            ...
            {[
              {
                id: "magister" as Bron,
                label: "Magister CSV",
                desc: "Exporteer leerlingenlijst als CSV uit Magister",
                emoji: "🎓",
              },
              {
                id: "somtoday" as Bron,
                label: "Somtoday Export",
                desc: "Importeer via Somtoday XML/CSV export",
                emoji: "📋",
              },
              {
                id: "csv" as Bron,
                label: "Handmatig CSV",
                desc: "Upload een eigen CSV-bestand",
                emoji: "📁",
              },
            ].map((opt) => (
              <button
                key={opt.id}
                onClick={() =>
                  opt.id !== "csv" ? laadVoorbeeldData(opt.id) : (setBron(opt.id), setStap(2))
                }
                    ...
                    <tr key={k}>
                      ...
                      <td className="py-2 pr-4">
                        <select
                          ...
                          onChange={(e) =>
                            setMapping((m) => ({ ...m, [k]: e.target.value as Kolom }))
                          }
          ...
          {!klaar ? (
              ...
              <Card title="Samenvatting">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">Leerlingen</div>
                  </div>
                  <div>
                  ...
                  <div>
                    <div className="text-2xl font-bold">{previewData.length || 3}</div>
                    <div className="text-xs text-muted-foreground">Ouder e-mails</div>
                  </div>
                </div>
              </Card>
              ...
              <Card title="Ouder koppeling">
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    Elke ouder ontvangt automatisch een persoonlijke koppelcode via e-mail. Na
                    inloggen met de code worden ze direct aan hun kind gekoppeld.
                  </p>
                  <div className="rounded-xl border border-border bg-muted/30 p-4">
                    ...
                    <div className="space-y-2 rounded-lg border border-border bg-background p-4 text-xs">
                      <div>
                        <strong>Van:</strong> noreply@schoolpulse.nl
                      </div>
                      <div>
                        <strong>Aan:</strong> p.devries@email.nl
                      </div>
                      <div>
                        <strong>Onderwerp:</strong> Uw toegang tot Schoolpulse — Sanne de Vries
                      </div>
                      <hr className="border-border" />
                      <p>Beste Petra de Vries,</p>
                      <p>
                        Uw kind Sanne de Vries is ingeschreven op onze school. Met de onderstaande
                        koppelcode krijgt u toegang tot het Schoolpulse ouderportaal.
                      </p>
                      <div className="my-2 rounded-lg bg-primary/10 p-3 text-center font-mono text-base font-bold text-primary">
                        SV2026
                      </div>
                      <p className="text-muted-foreground">
                        Ga naar schoolpulse.nl en voer uw code in om te beginnen.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-col items-center gap-4 rounded-2xl border border-success/30 bg-success/5 p-12 text-center">
              ...
              <div className="text-sm text-muted-foreground">
                3 leerlingen succesvol geïmporteerd. Ouders ontvangen hun koppelcode per e-mail.
              </div>
              <div className="mt-2 grid grid-cols-3 gap-6">
                ...
                <div>
                  <div className="text-2xl font-bold text-success">3</div>
                  <div className="text-xs text-muted-foreground">E-mails verstuurd</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-success">1</div>
                  <div className="text-xs text-muted-foreground">Klas</div>
                </div>
              </div>
              <button
                onClick={() => {
