...
function LeerlingCijfers() {
  ...
  const alle = cijfers
    .flatMap((c) => c.toetsen.map((t) => ({ ...t, vak: c.vak })))
    ...
    <AppShell title="Cijfers" subtitle="Schooljaar 2025-2026">
      ...
      <div className="mb-6 inline-flex rounded-lg border border-border bg-card p-1">
        <button
          ...
          className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "recent" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          Recent
        </button>
        <button
          onClick={() => setTab("overzicht")}
          className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "overzicht" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          Overzicht
        </button>
        <button
          onClick={() => setTab("prognose")}
          className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "prognose" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          Prognose
        </button>
      </div>
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-muted-foreground">
                    {t.naam} · ×{t.weging} · {t.datum}
                  </div>
                </div>
                <div
                  className={`shrink-0 rounded-lg px-3 py-1.5 text-lg font-bold ${t.cijfer < 5.5 ? "bg-destructive/15 text-destructive" : t.cijfer >= 8 ? "bg-success/15 text-success" : "bg-muted"}`}
                >
                  {t.cijfer.toFixed(1)}
                </div>
              </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4">
        <label className="shrink-0 text-sm font-semibold text-muted-foreground">
          Streefcijfer:
        </label>
        <select
          value={target}
          ...
          <div key={c.vak} className="rounded-2xl border border-border bg-card p-4">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="text-xs text-muted-foreground">
                Huidig gemiddelde:{" "}
                <span className="font-bold text-foreground">{gem.toFixed(2)}</span>
              </div>
            </div>
            {doelGehaald ? (
              ...
              <span
                className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${benodigd > 8 ? "bg-warning/15 text-warning" : "bg-primary/10 text-primary"}`}
              >
                Haal een {Math.max(1, benodigd).toFixed(1)} voor je volgende toets (weging{" "}
                {volgendeWeging})
              </span>
            )}
          </div>
...
function Overzicht({ filterVak, filterPeriode }: { filterVak: string; filterPeriode: string }) {
  ...
  const cijfersFiltered = cijfers
    ...
    .map((c) => ({
      ...c,
      toetsen: c.toetsen.filter(
        (t) => filterPeriode === "alle" || getPeriode(t.iso) === filterPeriode,
      ),
    }))
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-xs text-muted-foreground">
        <span className="font-semibold text-foreground">Voldoende-norm: 5.5.</span> Onder deze grens
        tel je als tekort. Hieronder zie je per vak wat je nog nodig hebt om weer voldoende te
        staan.
      </div>
        ...
        return (
          <div key={c.vak} className="rounded-2xl border border-border bg-card p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className={`text-lg font-bold ${isOnvold ? "text-destructive" : ""}`}>
                {gem.toFixed(2)}
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {c.toetsen.map((t, i) => (
                <span
                  key={i}
                  className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-semibold ${t.cijfer < 5.5 ? "bg-destructive/15 text-destructive" : t.cijfer >= 8 ? "bg-success/15 text-success" : "bg-muted"}`}
                >
            ...
            <div
              className={`mt-3 rounded-lg p-3 text-xs ${isOnvold ? "bg-destructive/5 text-destructive" : "bg-success/5 text-success"}`}
            >
              {isOnvold ? (
                <>
                  Je moet minimaal <strong>{nodig.toFixed(1)}</strong> halen op je volgende toets
                  (weging 1) om weer op {norm.toFixed(1)} te komen.
                </>
              ) : (
...
function DocentCijfers() {
  ...
  const [data, setData] = useState(
    () => JSON.parse(JSON.stringify(docentKlassen)) as typeof docentKlassen,
  );
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data.map((k, i) => (
            ...
            >
              ...
              <div className="text-xs text-muted-foreground">
                {k.vak} · {k.leerlingen.length} leerlingen
              </div>
            </button>
          ))}
        </div>
  ...
  if (leerlingId === null) {
    return (
      <AppShell title={`Cijfers · ${klas.klas}`} subtitle={klas.vak}>
        <button
          onClick={() => setKlasIdx(null)}
          className="mb-4 inline-flex items-center gap-1 text-xs font-semibold text-primary"
        >
          <ArrowLeft className="h-3.5 w-3.5" /> Terug naar klassen
        </button>
        <div className="mb-4 inline-flex rounded-lg border border-border bg-card p-1">
          <button
            onClick={() => setDocentTab("leerlingen")}
            className={`rounded-md px-3 py-1.5 text-xs font-semibold ${docentTab === "leerlingen" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
          >
            Leerlingen
          </button>
          <button
            onClick={() => setDocentTab("snel")}
            className={`rounded-md px-3 py-1.5 text-xs font-semibold ${docentTab === "snel" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
          >
            Snel invoeren
          </button>
        </div>
          ...
          <Card title="Leerlingen">
            <div className="space-y-2">
              {klas.leerlingen.map((l) => {
                ...
                const gem = c.length
                  ? c.reduce((a, t) => a + t.cijfer * t.weging, 0) /
                    c.reduce((a, t) => a + t.weging, 0)
                  : 0;
                return (
                  <button
                    key={l.id}
                    onClick={() => setLeerlingId(l.id)}
          ...
          <div className="space-y-2">
            {cs.map((t, i) => (
              ...
              >
                ...
                <div
                  className={`shrink-0 rounded-lg px-3 py-1.5 text-lg font-bold ${t.cijfer < 5.5 ? "bg-destructive/15 text-destructive" : "bg-muted"}`}
                >
                  {t.cijfer.toFixed(1)}
                </div>
                <button
                  onClick={() => setModal({ leerling, index: i })}
    ...
    <Card title="Snel invoeren">
      <div className="mb-4 space-y-3">
        <input
          ...
          placeholder="Toetsnaam (bijv. SO Hoofdstuk 5)"
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className="w-32 px-4 py-2 text-right">Cijfer</th>
            </tr>
          </thead>
          <tbody>
          ...
          onClick={() => {
            ...
            setData((d) => {
              ...
              klas.leerlingen.forEach((l, li) => {
                ...
                if (!val) return;
                const vak = klas.vak;
                if (!nd[klasIdx].leerlingen[li].cijfers[vak])
                  nd[klasIdx].leerlingen[li].cijfers[vak] = [];
                nd[klasIdx].leerlingen[li].cijfers[vak].push({
                  naam: toetsnaam,
                  cijfer: parseFloat(val),
                  weging: 1,
                  datum: nu,
                });
              });
...
function CijferModal({
  init,
  onSave,
  onClose,
}: {
  init: Cijfer | null;
  onSave: (c: Cijfer) => void;
  onClose: () => void;
}) {
      ...
      >
        <div className="flex items-center justify-between border-b border-border p-4">
          <div className="text-sm font-semibold">
            {init ? "Cijfer bewerken" : "Cijfer toevoegen"}
          </div>
        ...
        <div className="space-y-3 p-4">
          <Field label="Toets">
            <input
              ...
              className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary"
              placeholder="Bijv. SO Hoofdstuk 4"
            />
          </Field>
            ...
            <div key={k.klas} className="rounded-xl border border-border bg-background p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {k.leerlingen} leerlingen · {k.aanwezigheid}% aanwezigheid
                  </div>
                </div>
                <div
                  className={`text-2xl font-bold ${k.gemiddelde < 6 ? "text-destructive" : k.gemiddelde >= 7.5 ? "text-success" : ""}`}
                >
              ...
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted">
                <div
                  ...
                  style={{ width: `${(k.gemiddelde / 10) * 100}%` }}
                />
              </div>
            </div>
          ))}
